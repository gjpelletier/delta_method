...
- f = user-defined regression lambda (Python) or @ (MATLAB) function to predict y given inputs of parameters and x values (e.g. observed x or x_new) (Note that the Python version also requires a second input function that describes the same equation but using the Python def method for scipy as explained in the Jupyter notebook example)
- x = observed x
- y = observed y
