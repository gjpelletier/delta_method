...
# Nonlinear regression using lmfit combined with using the delta method 
...
This script uses the python package called lmfit for nonlinear regression. 
We also include a new function using the delta method, to extend beyond the capabilities of lmfit, 
...
In this example we use a 4-parameter logistic function with a sigmoid shape to fit an observed data set. 
The data set that we use provided by the R base package datasets, and consist of the waiting time 
between eruptions and the duration of the eruption for the Old Faithful geyser in Yellowstone National Park, Wyoming, USA.
...
"""Next we will use an example provided by the R base package datasets, and consist of the waiting time 
between eruptions and the duration of the eruption for the Old Faithful geyser in 
Yellowstone National Park, Wyoming, USA. 
In this example, x= duration of the eruption (minutes), and y= waiting time between eruptions (minutes)."""
...
"""We will use the ExpressionModel function of lmfit to specify the 4-parameter logistic function 
with a sigmoid shape that we want to fit to our data:"""

mod = ExpressionModel('(A-S) / ( 1 + exp(-gamma * (x - tau)) ) + S')

"""Now we can build set the initial parameter values as follows using guesses 
for the inital parameter values of A=90, gamma=2, tau=2, S=50:"""

pars = fit.Parameters()
pars['A'] = fit.Parameter(name='A', value=90, min=-np.inf, max=np.inf)
pars['gamma'] = fit.Parameter(name='gamma', value=2, min=-np.inf, max=np.inf)
pars['tau'] = fit.Parameter(name='tau', value=2, min=-np.inf, max=np.inf)
pars['S'] = fit.Parameter(name='S', value=50, min=-np.inf, max=np.inf)
...
"""The best-fit values of the model parameters can now be extracted into a numpy array that we named "param" 
from the lmfit results as follows:"""
...
"""Now we have almost everything we need in order to estimate the confidence intervals and prediction intervals of the model. 
Before doing that, we need to define a function that will apply the delta method as follows:"""
...
def delta_method(COVB,param,x_new,f,x,y,alpha):
    ...
    h = 1e-8       # use 1e-8 change for each param to calculate the derivative gradient
...
and the 95% prediction limits (lwr_pred and upr_pred), and 95% confidence limits (lwr_conf and upr_conf), 
...
"""Now we can plot the results of lmfit, combined with the results of the delta_method function to estimate 
the prediction intervals and confidence intervals."""
