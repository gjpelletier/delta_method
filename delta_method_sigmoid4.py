# -*- coding: utf-8 -*-
"""delta_method_sigmoid4_v28.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Tyvc91f1zjMr-zKTD-9PowHDWWofqDro
...
plt.text(1.0, 88, 'std err of the regression = '+ syxstr_, fontsize = 8)
plt.ticklabel_format(style='plain', axis='y')
# plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
...
# - - -
# get the number of observations, t-statistic, and number of new x-values
nobs = d_['nobs']
qt = d_['qt']
n_new = np.size(x_new)

# - - -
# define a function for the model to use for bootstrapping
# make sure this function is consisent with the previous model functions
def f_hat(param, xval):
    return (param[0]-param[3])/(1+exp(-param[1]*(xval-param[2])))+param[3]

# - - -
# a few more things that we need before doing the Monte Carlo
beta_hat = popt_                  # reference optimum parameters
rq = qt/stats.norm.ppf(1-alpha/2) # ratio of t-score to normal-score
y_hat_ref = f_hat(beta_hat, x)    # reference predicted y_hat at x
f_new = f_hat(beta_hat,x_new)     # reference predicted y_new at x_new
...
for i in range(trials):
    y_b = y_hat_ref + syx_ * stats.norm.rvs(size=nobs)
    popt_b, pcov_b = opt.curve_fit(f_, x, y_b, p0=popt_, bounds=(-np.inf,np.inf))
    f_b = f_hat(popt_b, x_new)
    res_f_hat[:,i] = f_b
    res_y_hat[:,i] = f_b + stats.norm.rvs(loc=0,scale=syx_,size=1)
...
plt.plot(x, y, '.', label='observations')
# plt.plot(x_new, y_new, '--k', label='best-fit (lmfit)',linewidth=4)    # lmfit solution of best fit
plt.plot(mc_x, mc_f, '-', label='best-fit (scipy)')     # scipy solution of best fit
# 95% prediction limits
plt.fill_between(mc_x, mc_lwr_pred, mc_upr_pred,color="#d3d3d3", label='95% PI (bootstrap)')
# 95% confidence limits
plt.fill_between(mc_x, mc_lwr_conf, mc_upr_conf,color="#ABABAB", label='95% CI (bootstrap)')
plt.legend(loc='lower right')
