# -*- coding: utf-8 -*-
"""delta_method_asympt3_v23.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IuBotQxMyEVpqF-WYflr0m_FAo4uaGnu
...
to estimate confidence intervals for predicted values, and prediction intervals for new data, using the nonlinear regression fit.
...
In this example we use a 3-parameter asymptotic exponential function to fit an observed data set 
...
"""We will use the ExpressionModel function of lmfit to specify the 3-parameter asymptotic exponential function 
that we want to fit to our data:"""

mod = ExpressionModel('b1 + b2 * exp(b3 * x)')

"""The next step is to define the initial values are ranges for the three parameters (b1, b2, b3) of the model function. 
We will assume that the intial values for b2 and b3 are zero, and the intial value for b1 is the 10th percentile 
of the observed y values. In other words, the intial guess for the model result is a constant value 
...
"""Now we have almost everything we need in order to estimate the confidence intervals 
and prediction intervals of the model. Before doing that, we need to define a function 
...
def delta_method(COVB,param,x_new,f,x,y,alpha):
    ...
    h = 1e-8       # use 1e-8 change for each param to calculate the centered derivative gradient
...
and the 95% prediction limits (lwr_pred and upr_pred), and 95% confidence limits (lwr_conf and upr_conf), at alpha=0.05 for new observations (x_new)"""
